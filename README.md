...
- **마지막 알**: 당길수록 작은 빛과 최대 세 번의 짧은 공명음이 차오른다. 게이지/타이밍 실패는 없다. 완료 저장은 POP 즉시. 모든 비즈가 도착하고 병이 정착한 뒤 완료음·`다 비웠다.`와 작은 별 6개가 약 1초간 나오며, 650ms 뒤 NEXT 버튼이 열린다. 마지막 보물이 보물함으로 날아가는 경우도 동일하다.
