...
- **마지막 알**: 완료 저장은 POP 즉시. 화면의 완료음·`다 비웠다.`는 모든 비즈가 도착하고 병이 정착한 뒤 나오며, 650ms 뒤 NEXT 버튼이 열린다. 마지막 보물이 보물함으로 날아가는 경우도 동일하다.
...
src/game/
  ...
  physics/pull.ts     붙잡힘(tension, 젤만 늘어남) → 미끄러짐(stick-slip, 큰 비즈는 중간에 1~2번 낑김) → POP(작은 비즈 외엔 속도 필요) / 미끄러져 빠짐·되들어감 상태기계
