...
- **마지막 알**: 당길수록 작은 빛과 최대 세 번의 짧은 공명음이 차오른다. 게이지/타이밍 실패는 없다. 완료 저장은 POP 즉시. 모든 비즈가 도착하고 병이 정착한 뒤 완료음·`다 비웠다.`와 작은 별 6개가 약 1초간 나오며, 1200ms 뒤 NEXT 버튼이 열린다. 마지막 보물이 보물함으로 날아가는 경우도 동일하다.
...
- **조명 캐시**: 기존 320×320 계산/색을 유지하고, 실제 외곽·구멍·크기·두께·색이 같은 결과를 최대 6장 재사용한다. 첫 생성 비용은 남는다. 검증 기록은 [마무리·입력 점검](docs/2026-09-12-round-finish.md).
...
- **토스 프로모션**: 첫 패드 완료 1원, 누적 3패드 완료 3원, 완료 이력이 있는 사용자의 다른 날 재방문 2원을 `Promotion.grantReward`로 지급한다. 기본 QA 빌드는 `TEST_` 코드만 사용하며, AIT 콘솔의 일반 테스트 푸시로 열면 세 코드를 한 번씩 자동 검증한다. 실제 지급 번들은 반드시 `VITE_PROMOTION_MODE=live pnpm ait:build`로 별도 생성한다. 로컬 저장소에 코드별 성공을 기록해 같은 설치에서 중복 호출하지 않고, SDK 미지원·지급 실패는 플레이를 막지 않는다.
- **배너 광고**: 현재 미연결. 설정·컬렉션 시트나 플레이 영역과 겹치지 않는 별도 배치가 확정되기 전에는 노출하지 않는다.
