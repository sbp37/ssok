...
function getSocketSprite(r: number, dpr: number) {
...
export class Gel {
  ...
  renderBase(inner: (ctx: CanvasRenderingContext2D) => void) {
    ...
    ctx.strokeStyle = this.col(0.34, -0.65);
    ctx.stroke();
    ctx.restore();
  ...
  drawSocket(ctx: CanvasRenderingContext2D, x: number, y: number, r: number) {
    const sp = getSocketSprite(r, this.texDpr);
  ...
  drawSurface(ctx: CanvasRenderingContext2D) {
    ...
    const blobs: [number, number, number, number, number, number][] = [
      [-0.3, -0.42, 0.55, 0.3, -0.55, 0.26],
      [-0.05, -0.55, 0.32, 0.18, -0.2, 0.16],
      [-0.5, -0.1, 0.22, 0.4, 0.4, 0.12],
    ];
    ...
    streak(-R * 0.34 + wx * 1.5, -R * 0.52 + wy * 1.5, R * 0.28, R * 0.05, 0.8);
    streak(-R * 0.1 + wx * 1.5, -R * 0.4 + wy * 1.5, R * 0.09, R * 0.025, 0.5);
    ctx.restore();
  }
}
